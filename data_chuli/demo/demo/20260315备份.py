...
) -> Tuple[Optional[float], Optional[float], Optional[Dict[str, str]], Optional[Dict[str, str]], Optional[str]]:
    """
    计算概率并生成预览图和原始图

    Returns:
        (head_prob, tail_prob, previews, original_images, error)
    """
    try:
        _init_models()
        if _CROPPER is None or _HEAD_MODEL is None or _TAIL_MODEL is None:
            return None, None, None, None, "models not initialized"
        ...
        return float(head_prob), float(tail_prob), previews, original_images, None
    except Exception as e:
...
def predict() -> Any:
    ...
    try:
        ...
        head_prob, tail_prob, previews, original_images, err = _compute_probs_and_previews_pil(img1, img2)
