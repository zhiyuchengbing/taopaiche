...
def _build_head_crop_context(crop_status: Optional[Dict[str, Any]]) -> str:
    ...
    return (
        ...
        "C4. 一侧车头裁切失败、一侧成功，通常是相机抓拍角度/距离等成像条件所致，往往一张可见完整车头、另一张仅见车头侧面（车门区域）："
        "采用同位对比原则，重点比对车门颜色、车门喷涂文字、后视镜总成形状与分色等稳定特征是否有冲突；"
        "无明显冲突 → 直接判 normal（reason 写明比对了车门/后视镜等特征一致）；"
        "不得因一张完整车头、一张侧面，或画面尺度不同就判套牌，也不得写「全景 vs 特写不可比 / 图片质量太差，AI无法判断」回避判断（C4 优先于 C3/P3）。\n\n"
        "【全景 vs 特写规则】\n"
...
class VehicleCheck:
    ...
    def _build_head_prompt(
        ...
        return (
            ...
            "一句到两句中文；触发 C2 无目标车辆时 label=fake_plate；车头裁剪不对称且失败侧可见车门侧面（C4）时按同位对比车门/后视镜定 normal，禁止走 P3 unknown；仅真全景vs特写（C3/P3）时才写输入图片质量太差、AI无法判断，label=unknown；"
