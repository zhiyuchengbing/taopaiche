...
def load_dataset(dataset_path, train_own_data, train_ratio):
    types       = 0
    # 检查是否存在images_background子目录，如果不存在则直接使用dataset_path
    images_bg_path = os.path.join(dataset_path, 'images_background')
    if os.path.exists(images_bg_path):
        train_path = images_bg_path
    else:
        train_path = dataset_path
        print(f"📁 未找到images_background目录，直接使用数据集路径: {train_path}")
